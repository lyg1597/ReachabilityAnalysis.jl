...
@taylorize function drone_10d!(dx, x, params, t)
    ...
    dx[3+1]=-31.6227766*x[0+1] + 31.6227766*x[10+1] - 8.81028908e-17*x[11+1] + 5.67884552e-16*x[12+1] - 45.1663196*x[1+1] - 52.5011267*x[2+1] - 13.6015533*x[3+1] + 8.81028908e-17*x[4+1] - 4.23825517e-16*x[5+1] - 6.54099623e-16*x[6+1] + 1.59750882e-16*x[7+1] - 5.67884552e-16*x[8+1] - 1.48006779e-15*x[9+1];
    dx[4+1]=x[5+1];
    dx[5+1]=9.81*tan(x[6+1]);
    dx[6+1]=-8*x[6+1] + x[7+1];
    dx[7+1]=2.05880071e-15*x[0+1] - 2.05880071e-15*x[10+1] + 10.0*x[11+1] - 1.9321227e-15*x[12+1] + 2.64242752e-15*x[1+1] + 2.62071672*x[2+1] + 1.59750882e-16*x[3+1] - 10.0*x[4+1] - 18.322492*x[5+1] - 24.6496768*x[6+1] - 11.3709874*x[7+1] + 1.9321227e-15*x[8+1] + 2.67619048e-15*x[9+1];
    dx[8+1]=x[9+1];
    dx[9+1]=-5.0652413175e-17*x[0+1] + 5.0652413175e-17*x[10+1] - 9.306933363e-17*x[11+1] + 0.91*x[12+1] - 6.7108633774e-17*x[1+1] - 1.194713975e-16*x[2+1] - 1.2256441379e-17*x[3+1] + 9.306933363e-17*x[4+1] + 1.2985133525e-16*x[5+1] + 1.0396072232e-16*x[6+1] + 2.2161533394e-17*x[7+1] - 0.91*x[8+1] - 1.6272983727*x[9+1];
    dx[10+1]=zero(x[10+1]);
    dx[11+1]=zero(x[11+1]);
    dx[12+1]=zero(x[12+1]);
    return dx
...
function drone_10d()
    ## initial states
    X0 = Hyperrectangle(
        low=[-1,0,0,0,-1,0,0,0,-1,0,4.8,-0.2,-0.2], 
        high=[1,0,0,0, 1,0,0,0, 1,0,5.2, 0.2, 0.2]
    ) 

    ## initial-value problem
    prob = @ivp(x' = drone_10d!(x), dim: 13, x(0) ∈ X0)
...
Wvel = 0.1
prob = drone_10d()
alg = TMJets(abstol=1e-7, orderT=5, orderQ=2, adaptive=true)
